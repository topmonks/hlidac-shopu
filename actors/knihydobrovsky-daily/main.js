...
import { cleanPrice, saveUniqProducts } from "@hlidac-shopu/actors-common/product.js";
...
function handleStart(document) {
  return document
    .querySelectorAll("#main div.row-main li a, #main div.row-main p a:has(img)")
    .map(a => a.href)
    ...
    .map(item => {
      ...
      const itemId =
        item
          ...
          ?.getAttribute("href")
          ?.match(/-(\d+)$/)?.[1] ??
        canonicalUrl(item.querySelector("a.buy-now")?.getAttribute("data-link"))?.searchParams?.get(
          "categoryBookList-itemPreview-productId"
        );
      if (!itemId) {
        log.warning("Could not find item id");
        return;
      }
      const discounted = item.querySelectorAll("span.flag-red").filter(x => x.innerText?.trim() === "AKCE").length > 0;
      return {
        itemId,
        ...
        currency: "CZK",
        inStock: item.querySelector("a.buy-now")?.innerText?.includes("Do košíku") ?? false,
        category: "",
        breadCrumbs: ""
      };
    })
...
function extractProductFromDetail({ document, url }) {
  ...
  const originalPrice = cleanPrice(productEl.querySelector(".price-before")?.innerText) ?? null;
  const currentPrice = productEl.querySelector("p.price strong")?.innerText?.trim()?.toLowerCase();
  ...
  return {
    ...
    rating: parseFloat(
      productEl.querySelector(".stars span[style]")?.getAttribute("style")?.split("width: ")[1] ?? null
    ),
    currency: "CZK",
    inStock: productEl.querySelector("a.buy-now")?.innerText?.includes("Do košíku") ?? false,
    category: "",
    breadCrumbs: ""
  };
}
...
async function main() {
  ...
  const { development, maxRequestRetries, proxyGroups, type = ActorType.Full, bfUrls = [] } = await getInput();
  ...
  const crawler = new HttpCrawler({
    ...
    async requestHandler({ request, body, crawler }) {
      ...
      switch (label) {
        ...
        case "SUBLIST":
          stats.inc("pages");
          log.info(`Adding pagination page ${request.url}?sort=2&currentPage=1`);
          await crawler.requestQueue.addRequest(
            {
  ...
  if (!development) {
    try {
      await uploadToKeboola(type === ActorType.BlackFriday ? "knihydobrovsky_cz_bf" : "knihydobrovsky_cz");
