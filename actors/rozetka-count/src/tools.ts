...
import { NAMED_KV_STORE, STATS_KEY } from './consts';
...
export function migrationFlagGetOrSet(value?: boolean) {
    if (value === undefined) {
        return isMigrating;
    }
    else {
...
let statsValue = 0;
export const getOrIncStatsValue = async function(inc?: number, url?: string) {
    if (inc !== undefined) {
        statsValue += inc;
