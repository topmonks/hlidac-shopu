...
const ProductPriceByTags = (() => {
    ...
    if (hasBadWithoutVat) {
      ...
      for (const tag of mktTagsHvezda) {
        if (tag.includes(tagBadPercentPrefix)) {
          badPercentTags.push(parseInt(tag.replace(tagBadPercentPrefix, "").replace(/\s/g, "")));
        } else if (tag.includes(tagBadFixedPrefix)) {
          badFixedTags.push(parseInt(tag.replace(tagBadFixedPrefix, "").replace(/\s/g, "")));
        }
      }
      ...
      if (productDetail && isDiscountApplied) {
        ...
        if (!isPriceAtLowest(price * 100)) {
          discountBarContent = Shopify.translation.enhanced_price.replace(/<sup>.+<\/sup>/gi, "");
  ...
  const generateMktInfobox = message => {
    ...
    $("body.product .product__information .is-gray-around:eq( 0 )").before(`<div class="mkt-infobox">${message}</div>`);
  };
  ...
  const handleFrontendDiscountsDueToLaws = ({
    ...
    if (!filterProductsByStore && comparablePrice > productPriceWithDiscount) {
      ...
      const hasDphDiscountTag = updatedProductData.tags.some(
        tag => tag.includes(Shopify.theme_settings.tag_bad_without_vat) && tag.includes("★")
      );
  ...
  const isPriceAtLowest = price => {
    ...
    const priceHistory = priceHistoryElement.dataset.values.split(",").map(price => parseInt(price));
  ...
  const updateComparePrice = (
    ...
    comparePriceContainer.innerHTML =
      `<span class="title">${lowestPriceIn30Text} <sup>*2</sup></span>` +
