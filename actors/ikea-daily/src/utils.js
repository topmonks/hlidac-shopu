...
exports.getSubcategoriesUrls = ($) => {
    const subcategories = [];
    $("nav[role='navigation'] a[class*='vn-link']")
        .each((index, link) => {
...
exports.getPrice = ($) => {
    const integer = $("div[class='range-revamp-pip-price-package__main-price']")
        .find("span[class='range-revamp-price__integer']").eq(0).text()
        .replace(/\s/, '');
    const decimals = $("div[class='range-revamp-pip-price-package__main-price']")
        .find("span[class='range-revamp-price__decimals']").eq(0)
...
exports.getVariantName = ($, productTypeName) => {
    ...
    const spans = $("h1[class='range-revamp-header-section']")
        .find("div[class='range-revamp-header-section__description']").eq(0).find('span');
...
exports.tryGetRetailPrice = ($) => {
    ...
    let integer = $("div[class='range-revamp-pip-price-package__previous-price-hasStrikeThrough']")
        .find("span[class='range-revamp-price__integer']").eq(0).text()
        .replace(/\s/, '');
    let decimals = $("div[class='range-revamp-pip-price-package__previous-price-hasStrikeThrough']")
        .find("span[class='range-revamp-price__decimals']").eq(0)
        .clone() // clone the element
        .children() // select all the children
        .remove() // remove all the children
...
exports.getReview = ($) => {
    ...
    const reviewButton = $("button[class='range-revamp-average-rating range-revamp-average-rating__button']");
...
exports.getProductDetailCategories = ($) => {
    const categories = [];
    $("li[class='bc-breadcrumb__list-item']").find('span')
        .each((index, category) => {
