...
const { getSubcategoriesUrls, fillProductData, tryGetRetailPrice,
    getReview, getProductDetailCategories, getPrice, getVariantName } = require('./utils');
    ...
    if (subcategories.length === 0) {
        try {
            ...
            await requestQueue.addRequest({
                url: `https://sik.search.blue.cdtapps.com/${countryPath}/product-list-page/more-products?category=${dataCategory.id}`
...
exports.handleList = async ({ request, body }, requestQueue) => {
    ...
    try {
        ...
        log.info(`[LIST]: ready to scrape ${products.length} products --- ${request.url}`);
    } catch (e) {
        log.info(`[LIST]: ${JSON.parse(body).reason}, removing from scraped pages --- ${request.url}`);
    }
    for (const product of products) {
        ...
        for (const variant of productVariants) {
            ...
            productData.variantName = variant.imageAlt.substr(
                variant.imageAlt.indexOf(productData.productTypeName) + productData.productTypeName.length + 2,
            );
            await requestQueue.addRequest({
                url: variant.pipUrl,
                userData: {
...
exports.handleDetail = async ({ $ }, productData) => {
    ...
    if (productData.originalPrice && productData.currentPrice < productData.originalPrice) {
    ...
    if (!productData.variantName) {
        productData.variantName = getVariantName($, productData.productTypeName) || '';
    }
    if (productData.variantName) {
