...
const productItemToken = '.product-list__item.product-box.product-box--tile.ajax-wrap';
...
const productCategoriesToken = '.breadcrumb__list.l-in-box.u-maw-1310px.ol--reset';

function mkPrice(price) {
    if (price !== '') {
        price = Number(encodeURIComponent(
            price.substr(0, price.length - 2)
        ).replace(/%C2/g, '').replace(/%A0/g, ''));
    } else {
        price = null;
    }
    return price;
...
async function scrapeProductListPage($, crawlContext) {
    ...
    for (let i = 0; i < products.length; i++) {
        ...
        const productName = topElement.find(productNameToken).text().trim().replace(
            new RegExp(String.fromCharCode(160), ''),''
        );

        let productPriceOriginal = topElement.find(productPriceOriginalToken).text();
        ...
        if (isNaN(productPrice)) {
            productPrice = topElement.find('.product-box__prices').find('.product-box__price')[0].children[0];
            productPrice = productPrice.data.replace(/\t/g,'').replace(/\n/g,'');
            productPrice = mkPrice(productPrice);

            productPriceOriginal = $(topElement).find(productPriceToken)[1].children[0].data;
            productPriceOriginal = productPriceOriginal.replace(/\t/g,'').replace(/\n/g, '').trim();
            productPriceOriginal = mkPrice(productPriceOriginal)
        }
        ...
        const productLink = `${urlBase}${topElement.find(productLinkToken).attr('href')}`;
        ...
        const avail1 = $(availabilityTop[0]).attr('class').includes('availability--available');
        const avail2 = $(availabilityTop[1]).find('span').attr('class') === 'availability--available';
...
async function addProductListPagesToQueue($, crawlContext, firstPageURL) {
    ...
    for (let i = 2; i < maxPages; i++) {
        ...
        await crawlContext.requestQueue.addRequest({userData: {label: 'nthPage', pageN: i}, url: url});
    }
}
...
exports.fetchPage = async ({request, $}, crawlContext) => {
    if (request.userData.label === 'nthPage') {
        ...
        if (isSubCategoryPage) {
            log.info(`Found new subcategory page: ${request.url}, ${crawlContext.subcategoryPageCount}`);
            await handleSubCategoryPage($, crawlContext, request);
            crawlContext.subcategoryPageCount++;
        } else {
            log.info(`Scraping 1st product list page: ${request.url}, ${crawlContext.productListPageCount}`);
