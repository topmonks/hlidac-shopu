...
import { ACTOR_TYPES, CATEGORY_LIST_ITEM_SELECTOR, LABELS,
    GAMER_GOODS_URL, RESTAURANT_GOODS_URL } from '../consts.js';
...
export const handleProductList = async ({
    ...
    const subcategoryUrl = $('.breadcrumbs li:nth-child(3) a')?.attr('href')?.trim();

    if (subcategoryUrl === GAMER_GOODS_URL
    ...
    if ($(CATEGORY_LIST_ITEM_SELECTOR).length > maxCategoryDepth) {
        await requestQueue.addRequest({
            url: new URL(
                $(`.breadcrumbs li:nth-child(${maxCategoryDepth + 2}) a`).attr('href')?.trim(),
                loadedUrl,
            ).href,
            userData: { label: LABELS.CATEGORY_OR_PRODUCTS },
        });

        log.info(`Too long or absent categories list, going to page ${new URL(
            $('.breadcrumbs li:nth-child(3) a').attr('href')?.trim(), loadedUrl,
        ).href}`);
        return;
    }
