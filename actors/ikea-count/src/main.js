...
Apify.main(async () => {
    ...
    Apify.events.on('migrating', () => {
        Apify.setValue('COUNT', productCount).then(() => log.info('[PRODUCT COUNT] Saved'));
    });
    ...
    const categoryRequests = await retry(async () => {
        // if anything throws, we retry
        const response = await requestAsBrowser({ url: sitemap, proxyUrl: proxyConfiguration.newUrl() });
