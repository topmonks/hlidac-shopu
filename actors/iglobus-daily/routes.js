...
exports.handleList = async ({ request, $ }) => {
    ...
    // add details
    for (link of $('.article-head').map(function () {
        return $(this).find('a').attr('href');
    }).get()) {
        requestQueue.addRequest({ url: `https://www.iglobus.cz${link}`, userData: { label: 'DETAIL' } });
    }
...
exports.handleDetail = async ({ request, $ }) => {
    ...
    if (discount) {
        result.originalPrice = Math.round(result.currentPrice * 100 / (100 - discount) * 10) / 10;
    } else {
