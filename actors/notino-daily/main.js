...
const { HOME_PAGE, BASE_URL, BF, BASE_URL_SK_BF, BASE_URL_CZ_BF, BASE_URL_SK, COUNTRY } = require('./src/consts');
const {extendCheerio, handlePageFunction, handleFailedRequestFunction } = require('./src/funcs');
...
Apify.main(async () => {
    ...
    log.debug(`test mode: ${(testMode)}, environment: ${(testEnv)}, country: ${(country)}`);
    ...
    if (type === BF) {
        ...
        await requestQueue.addRequest({ url: rootUrl, userData: { label: HOME_PAGE } });
    }
    // todo type === 'COUNT'
    ...
    const crawler = new Apify.CheerioCrawler({
        ...
        // eslint-disable-next-line max-len
            ({ request, $, session, response }) => handlePageFunction(requestQueue, request, extendCheerio($), session, response, input, proxyConfiguration, stats),
    ...
    // await persistState();
    await Apify.setValue('STATS', stats).then(() => log.debug(`STATS saved! ${JSON.stringify(stats)}`));
