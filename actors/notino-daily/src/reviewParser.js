...
const i = {
    ...
    query: 'query getReviews($page: Int!, $pageSize: Int!, $orderDesc: Boolean!, $orderBy: ReviewOrderBy!, $code: String!) {\n  reviews(page: $page, pageSize: $pageSize, orderDesc: $orderDesc, orderBy: $orderBy, code: $code) {\n    id\n    text\n    userName\n    score\n    createdDate\n    like\n    dislike\n    alreadyLiked\n    alreadyDisliked\n    __typename\n  }\n}\n',
};
...
async function getReviews({ sku, token, proxyConfiguration }) {
    ...
    for (let page = 1; 1 > 0; page += 1) {
        const reviewsRaw = await getReviewPage({ sku, token, page, proxyConfiguration });
        if (reviewsRaw && reviewsRaw.length !== 0) {
            for (const review of reviewsRaw) {
                reviews.push(review);
            }
            log.info(`Found ${reviewsRaw.length}, loading more with page ${page}, ${sku}`);
