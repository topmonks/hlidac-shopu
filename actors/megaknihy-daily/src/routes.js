...
const { getAllSubcategories, getMaxPaginationNumber, getProductsData } = require('./utils');
...
exports.handleStart = async ({ url, $, body }, requestQueue) => {
    ...
    if (subcategories.length === 0) {
        ...
        throw new Error(`Handling of starting url ${url} failed, look at the HTML in key-value store.`);
    }
};

exports.handlePage = async ({ url, $, body }, categories, requestQueue, page, alreadyScrapedProducts) => {
    ...
    const { productsData, outOfStockProductDiscovered } = getProductsData($, categories, alreadyScrapedProducts);
    ...
    if (outOfStockProductDiscovered) {
        ...
        log.info(`[PAGE_FINISH]: Found ${pageNumber} pages with products in stock for ${categories} --- ${url}`);
    } else {
        const maxPagination = Number(getMaxPaginationNumber($));
        if (maxPagination === 0) {
            // there is only one page
            log.info(`[PAGE_FINISH]: Found ${pageNumber} page with products in stock for ${categories} --- ${url}`);
        } else if (Number(pageNumber) < maxPagination) {
            await requestQueue.addRequest({
                url: `${url.substr(0, url.length - (3 + pageNumber.length))}?p=${Number(pageNumber) + 1}`,
                userData: {
                    label: 'PAGE',
                    categories,
                },
