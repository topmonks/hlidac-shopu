exports.getAllSubcategories = ($) => {
    ...
    const subcategories = [];
    const menu = $("div[id='categories_block_left']").find("ul[class*='tree']").eq(0);
    $(menu).find("> li:not([class*='action'])")
        .each((index, category) => {
            ...
            if ($(category).has('ul').length !== 0) {
                $(category).find('li')
                    .each((index2, subcategory) => {
                        subcategories.push({
                            name: [categoryName, $(subcategory).find('a').eq(0).attr('title')],
...
exports.getMaxPaginationNumber = ($) => $("li[id='pagination_next']").prev().eq(0).text();
...
const modelProductData = ($, product, categories) => {
    ...
    const productData = {
        itemId: $(title).attr('href').match(/\/([0-9]){4}[0-9]*-/)[0].match(/[0-9]+/)[0],
        itemImg: $(product).find('img').eq(0).attr('src'),
        itemUrl: $(title).attr('href'),
        itemName: $(title).text(),
        currentPrice: parseInt($(bottomBox).find("span[class='price']").eq(0).text()
            .match(/[0-9]+/)[0], 10),
        originalPrice: $(bottomBox).find("span[class='old-price']").eq(0).text(),
        rating: $(bottomBox).find("span[class='rating']")
            .eq(0).text().trim(),
        discounted: false,
        sale: $(product).find("div[class*='product-sale']").eq(0).text(),
        category: categories,
        currency: $(bottomBox).find("span[class='price_currency']").eq(0).text() === 'Kč'
            ? 'CZK' : $(bottomBox).find("span[class='price_currency']").eq(0).text(),
...
exports.getProductsData = ($, categories, alreadyScrapedProducts) => {
    ...
    products.each((index, product) => {
        ...
        if (productData.originalPrice) {
            productData.originalPrice = parseInt(productData.originalPrice.match(/[0-9]+/)[0], 10);
        } else {
        ...
        if (productData.rating) {
            productData.rating = parseFloat(productData.rating.match(/[0-9]+\.?[0-9]*/)[0]);
        }
