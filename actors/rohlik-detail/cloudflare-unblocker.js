...
/** @typedef { import("apify").SessionPool } SessionPool */

class CloudflareUnblocker extends CrawlerExtension {
  /**
   *
   * @param options - Cloudflare unblocker options
   * @param options.proxyConfiguration {ProxyConfiguration} - Proxy groups from the Apify platform
  ...
  constructor(options) {
    ...
    this.puppeteerPool = new BrowserPool({
      retireBrowserAfterPageCount: 50,
      maxOpenPagesPerBrowser: 10,
      browserPlugins: [
        new PlaywrightPlugin(puppeteer, {
          launchOptions: { headless: false, useChrome: true }
        })
      ]
    });
  ...
  /**
   * Locks session
   * @TODO: we could add this function to the Session natively
