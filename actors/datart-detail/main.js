...
const parseHtmlData = $ => {
  ...
  //Thumbnail
  data.thumbnailUrl = $('meta[property="og:image"]')
    .attr("content")
    .split("?")[0];
  //Images
  let productImages = $("div#product-detail-image")
    .find("a")
    .toArray()
  ...
  //PropertyValue
  let propertyValues = $("div#id-attributes table.list")
    .find("tr")
    .map((index, element) => {
...
const processData = data => {
  ...
  let result = {
    ...
    mainEntity: {
      ...
      brand: data.brand,
      category: data.breadcrumbs
        ? data.breadcrumbs.map(breadcrumb => breadcrumb.item.name).join(" > ")
        : "",
      offers: data.offers,
      sku: data.product.id,
...
Apify.main(async () => {
  ...
  const { development, country = "cz", extensions = [], maxConcurrency = 10 } =
    input || {};
