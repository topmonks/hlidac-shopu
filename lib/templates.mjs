...
import { html, svg, directive } from "lit-html/lit-html.js";
...
import { shopsArray } from "./shops.mjs";

export const when = directive((condition, trueContentProvider) => part => {
  if (condition) part.setValue(trueContentProvider());
});
...
export function shopsListTemplate() {
  return html`
...
export function resultsEmbed(url) {
  const parameters = new URLSearchParams({ url, view: "embed" }).toString();
  return html`
    <iframe
