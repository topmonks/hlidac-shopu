...
import { formatMoney, formatPercents } from "./format.js";
import { chartTemplate, defineStyles as chartStyles } from "./chart.js";
...
export function discountTemplate({ realDiscount: discount, type }) {
...
export function widgetTemplate(
  data,
  metadata,
  { showFooter, showLegend } = { showFooter: true, showLegend: true }
) {
  ...
  return html`
    ...
    <div id="hlidacShopu">
      ...
      ${showFooter ? footerTemplate() : null}
    </div>
  `;
}
...
export function logoTemplate({ logo, name, url, viewBox }) {
  const image = svg`
    <svg viewBox="${viewBox}">
      <title>${name}</title>
      <use href="#${logo}"></use>
