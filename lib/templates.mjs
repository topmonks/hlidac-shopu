...
import { defineStyles as chartStyles, chartTemplate } from "./chart.mjs";
import { formatMoney, formatPercents } from "./format.mjs";
import { supportedShops } from "./shops.mjs";
...
function widgetStyles() {
  return css`
    #hlidacShopu {
      font-family: "IBM Plex Sans Var", system-ui, sans-serif;
      font-kerning: normal;
...
export function discountTemplate({ realDiscount: discount, type, claimedDiscount }, showClaimedDiscount) {
  ...
  const discountClass = x => ({
    ...
    "hs-real-discount--good": x !== 0 && x === claimedDiscount
  });
  ...
  return html`
    <div class="hs-header__discount">
      ...
      ${when(showClaimedDiscount && discount !== claimedDiscount && claimedDiscount, () =>
  ...
  return html`
    ...
    <div id="hlidac-shopu-modal__not-found" class="hs-result">
      ...
      <section>
        ...
        ${shopsListTemplate(supportedShops())}
      </section>
    </div>
  `;
}
...
export function logoTemplate(shop) {
  ...
  return html`
    <a
      ...
      title="${name}"
      target="_blank"
      rel="noopener noreferrer"><img alt="${name}" src="/assets/img/${logo}.svg" loading="lazy"></a>
  `;
}
