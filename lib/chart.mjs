...
Chart.defaults.font.family = "'Montserrat', 'Work Sans', 'SF Pro', sans-serif";
...
const tooltipFormatter = (originalPriceLabel, currentPriceLabel) => ({
  title(items) {
    const date = new Date(items[0].element.x);
    return formatDate(date);
  },
  label(item) {
    if (item.datasetIndex === 0) {
      return `${originalPriceLabel}: ${formatMoney(item.element.y)}`;
    } else if (item.datasetIndex === 1) {
      return `${currentPriceLabel}: ${formatMoney(item.element.y)}`;
    }
  },
  labelColor(item) {
