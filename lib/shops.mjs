  ...
  [
    "e-coop",
    {
      ...
      parse(url) {
        return {
          itemId: null,
          itemUrl: url.pathname.match(/([^/]+)\.html$/)?.[1] ? url.pathname.match(/([^/]+)\.html$/)?.[1] : url.pathname.match(/s\/(.*)\/c/s)?.[1]
        };
      }
    }
  ],
...
export function shopName(s) {
  ...
  if (domain === 'uk' || domain === 'ua') {
    shopName = domainParts.pop(); // skip the 'co' from shopname.co.uk || 'com' from shopname.com.ua
  }
