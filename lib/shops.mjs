...
const dmCz = {
  ...
  /** @param {URL} url */
  parse(url) {
    // DM ships two product-URL formats: the legacy `/<slug>-p<id>.html`
    // and the current `/p/d/<id>/<slug>` after their detail-page rewrite.
    const newFormat = url.pathname.match(/^\/p\/d\/(\d+)(?:\/|$)/)?.[1];
    const legacy = url.pathname.match(/-p(\d+)\.html$/)?.[1];
    return {
      itemId: newFormat ?? legacy,
      get itemUrl() {
        return this.itemId;
      }
    };
...
const mojaDmSk = {
  ...
  /** @param {URL} url */
  parse(url) {
    // Same two product-URL formats as DM.cz: legacy `/<slug>-p<id>.html`
    // and the current `/p/d/<id>/<slug>` after their detail-page rewrite.
    const newFormat = url.pathname.match(/^\/p\/d\/(\d+)(?:\/|$)/)?.[1];
    const legacy = url.pathname.match(/-p(\d+)\.html$/)?.[1];
    return {
...
const smartyCz = {
  ...
  parse(url) {
    return {
      itemId: url.pathname.match(/-(?:\d+)?p(\d+)$/)?.[1],
...
const tsbohemiaCz = {
  ...
  parse(url) {
    return {
      itemId: url.pathname.match(/_d(\d+)(?:\.html)?$/)?.[1],
      get itemUrl() {
        return this.itemId;
      }
    };
...
export const shops = new Map([
  ...
  ["globusonline.cz", iGlobusCz],
  // `shopName(url)` derives `globusonline_cz` for the new domain — register
  // here so the extension and API both resolve the shop.
  ["globusonline", iGlobusCz],
  ["globusonline_cz", iGlobusCz],
  ["ikea_cz", ikeaCz],
  ["ikea.cz", ikeaCz],
  ["ikea_sk", ikeaSk],
  ["ikea.sk", ikeaSk],
...
/**
 * Gets unique entries of supported shops. Shops marked with `show: false`
 * are no longer monitored and are excluded from user-facing listings.
 * @returns {Generator<ShopDefinition>}
 */
export function* supportedShops() {
  const returned = new Set();
  for (const shop of shops.values()) {
    if (shop.show === false) continue;
