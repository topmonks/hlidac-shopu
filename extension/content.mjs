...
import {
  chartTemplate,
  createChart,
  defineStyles as chartStyles,
  getCanvasContext
} from "@hlidac-shopu/lib/chart.js";
      ...
      <div class="hs-header">
        <div>
          ...
          <div class="hs-original-price">
            ${metadata.type === "eu-minimum"
              ? "Minimální cena před akcí"
              : "Běžná cena"}
            <b
              >${metadata.type === "eu-minimum"
                ? formatMoney(metadata.minPrice)
                : formatMoney(metadata.commonPrice)}</b
            >
          </div>
        </div>
        ${discountTemplate(metadata)}
      </div>
      ${chartTemplate("Uváděná původní cena", "Prodejní cena")}
      <div class="hs-footer">
        <div>
          Více informací na
