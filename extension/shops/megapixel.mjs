...
export class Megapixel extends Shop {
  get injectionPoint() {
    return ["beforeend", ".product-detail__right"];
  }

  async scrape() {
    const elem = document.querySelector("#snippet--price");
    if (!elem) return;
    const itemId =
      document.querySelector("[data-product-item-id]")?.dataset.productItemId ??
      document.querySelector(".product-detail a[href^='/hlidaci-pes-produktu?item=']")?.href.split("=")[1];
